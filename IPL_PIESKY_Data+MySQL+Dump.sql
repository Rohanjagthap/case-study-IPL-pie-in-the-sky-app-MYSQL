...
select t3.user_id,t3.bidder_name,t1.BIDDER_ID,t1.bid_status,count(*) as 'total',(count(*)/t2.total_bids)* 100 as 'percentage'
from ipl_bidding_details t1
join
(select bidder_id,count(*) as 'total_bids' from
ipl_bidding_details where bid_status!='cancelled' group by bidder_id) t2
on t1.bidder_id=t2.bidder_id
join ipl_bidder_details t3
on t1.bidder_id=t3.bidder_id
where t1.BID_STATUS='won'
group by t1.bid_status,t1.BIDDER_ID
order by (count(*)/t2.total_bids)* 100 desc;

/*2.	Which teams have got the highest and the lowest no. of bids?*/
(select t.TEAM_NAME,bid_team,count(*) as 'No of bids' from ipl_bidding_details bdd
inner join ipl_team t
on bdd.BID_TEAM=t.TEAM_ID
...
select ms.stadium_id,s.stadium_name,count(*) as matches_won_toss,t1.total_matches,(count(*)/t1.total_matches) * 100 as 'percent of wins by toss'
from
ipl_match m
...
inner join
(select ms.stadium_id,s.stadium_name,count(*) as total_matches from
ipl_match m
inner join ipl_match_schedule ms
on m.match_id=ms.match_id
inner join ipl_stadium s
...
order by s.stadium_id) t1
on t1.stadium_id=ms.stadium_id

where m.match_winner=m.toss_winner
group by ms.stadium_id
order by s.stadium_id;

/*4.	What is the total no. of bids placed on the team that has won highest no. of matches?*/
select ibd.bid_team,it.team_name,count(*) as total_bids,t.total_wins from ipl_bidding_details ibd
join 

(select count(*) as total_wins,
case
when match_winner=team_id1 then team_id1
else team_id2
end win_teamid
from ipl_match
group by win_teamid
...
select count(*) as total_bids from ipl_bidding_details ibd
join 
...
/*5.	From the current team standings, if a bidder places a bid on which of the teams, there is a possibility of (s)he winning the highest no. of points – 
in simple words, identify the team which has the highest jump in its total points (in terms of percentage) from the previous year to current year.*/

select t.team_id,t.team_name,ts.TOURNMT_ID,ts.total_points,
lag(ts.total_points,1) over (partition by ts.team_id) as 'previous year points',
((ts.total_points-lag(ts.total_points,1) over (partition by ts.team_id))/lag(ts.total_points,1) over (partition by ts.team_id))* 100 
as 'percent_jump_in_points'

from ipl_team_standings ts
join ipl_team t
on ts.team_id=t.team_id
order by percent_jump_in_points
desc limit 1;
